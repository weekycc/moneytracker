...
namespace weekysoft.store.Util
{
    ...
    public class HttpValueCollection : Collection<HttpValue>
    {
        ...
        public string this[string key]
        {
            ...
            set
            {
                var pair = this.FirstOrDefault(x => string.Equals(x.Key, key, StringComparison.OrdinalIgnoreCase));
                if (pair == null)
                {
                    pair = new HttpValue(key, value);
                    this.Add(pair);
                }
                else
                {
        ...
        {
            var list = this.Where(x => string.Equals(x.Key, key, StringComparison.OrdinalIgnoreCase))
                .ToList();
            foreach (var x in list) { this.Remove(x); }
        }
